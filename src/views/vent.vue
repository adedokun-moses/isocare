<template>
  ...
  import addvent from "@/components/addvent.vue";
  import edit from '@/components/editvent.vue'
  export default {
    components: { top, side, addvent, edit },
    data() {
      return {
        search: ""
      };
        ...
        if (this.search) {
          return this.allorders.filter((data) => {
            return this.search
              .toLowerCase()
              .split(" ")
              .every((v) => data.ventilator_name.toLowerCase().includes(v));
          });
        } else {
